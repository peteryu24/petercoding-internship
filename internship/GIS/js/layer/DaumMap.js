...
window.myApp.maps = window.myApp.maps || {};
...
function createDaumMap() {
	...
	var map = new ol.Map({
		...
		/*
		 * WebGL (Web Graphics Library)을 사용하여 맵을 렌더링 canvas, dom 같은 renderer도
		 * 존재하지만 WebGL이 가장 효율적인 방법
		 */
		renderer : "webgl",
	...
	// 확대 레벨별 해상도
	var _daumResolutions = [ 2048, 1024, 512, 256, 128, 64, 32, 16, 8, 4, 2, 1,
			0.5, 0.25, 0.125 ];
	// 다음 지도 타일 레이어 생성
	var _daumMapLayer = new ol.layer.Tile({
		...
		source : new ol.source.XYZ({
			...
			tileUrlFunction : function(coordinate) {
				...
				/*
				 * 서브 도메인 값을 계산하여 여러 서버에서 타일을 로드할 수 있도록 분산 동시 다운로드 증가, 로딩 속도 향상
				 */
				var subdomain = ((level + col) % 4) + 1;
				// 최종 타일 url 반환
				return "http://map" + subdomain
						+ ".daumcdn.net/map_2d/1909dms/L" + level + "/" + row
						+ "/" + col + ".png";
			},
			// 타일 그리드 설정
